...
#### 📋 Required OKF v0.1 Front Matter Fields (Standard Documentation)

Every standard Markdown documentation file must start on line 1, column 1 with a YAML front matter block delimited by `---` and contain the following mandatory keys:
...
#### 📋 Required OKF v0.2 Front Matter Fields (Agent Skills & Knowledge Catalogs)

Agent Skills (`SKILL.md`) and sovereign knowledge catalogs are explicitly exempt from OKF v0.1 requirements and use OKF v0.2 front matter with the following keys:
1. `okf_version`: Set to `"0.2"` (as a double-quoted string).
2. `spec_version`: Set to `"0.2"` (as a double-quoted string).
3. `trust_pillars`: A YAML array of trust signal strings (e.g., `["Verifiable Architectural Context", "Deterministic Automation Standards", "Sovereign AI Memory Stratification"]`).
4. `type`: Document class (e.g., `"Agent Skill"`, `"Jules Knowledge Catalog"`).
5. `title`: Human-readable display name.
6. `timestamp`: ISO 8601 combined date and time format (e.g., `2026-08-13T12:00:00+08:00`).
7. `topics`: A YAML array of keywords.
...
- **Run Automatic Formatting:** Run `python3 scripts/prepare_docs.py` to automatically validate, clean, and format all standard OKF v0.1 front matter headers across standard documentation in the repository (note: `SKILL.md` files and knowledge catalogs use OKF v0.2 and are explicitly exempt from OKF v0.1 formatting rules).

---

### 5.2 Google Antigravity-Compatible Agent Skills (.agents/skills/ & skills/)

This project defines and supports the **Agent Skills open standard** (governed under [AgentSkills.io](https://agentskills.io/home) and [Google Antigravity Agent Skills](https://antigravity.google/docs/skills)) to share, version, and reuse operational knowledge between **Google Jules**, **Google Antigravity**, and other advanced AI agents.

#### 📂 Workspace Skills Structure & Dual-Path Synchronization

All custom skills are packaged under both `.agents/skills/<skill-folder>/` and synchronized to the root `skills/<skill-folder>/` directory to guarantee complete cross-platform compatibility across Google Antigravity, AgentSkills.io, Warp Agent Skills, and Google Jules agent runners. Each skill is a self-contained package consisting of a `SKILL.md` file:
- **`SKILL.md` format:** Combines OKF v0.2 frontmatter (`spec_version: "0.2"`, trust pillars) with standard Agent Skills metadata fields (`name` and `description`), containing detailed instructions, patterns, and scenarios to trigger the skill. Both fields must be explicitly retained.
...
#### 🛠️ Agent Skills Catalog & Operational Parameters

The workspace features a comprehensive suite of 6 agent-specific skills listed below:
...
Agents discover available skills at startup by reading both their `name` and `description` metadata fields. The `name` acts as a unique lowercase identifier, and `description` acts as the primary semantic search trigger. Under the Agent Skills open standard, when a user request matches a skill's intent, the agent activates the skill, reads the full `SKILL.md` instructions, and executes the tasks deterministically.
