...
metadata:
  ...
  spec_version: "0.2"
spec_version: "0.2"
trust_pillars: ['Verifiable Architectural Context', 'Deterministic Automation Standards', 'Sovereign AI Memory Stratification']
---
...
- When performing cost recalculations, always enforce the use of `Decimal` values constructed from strings and exact Banker's rounding via `ROUND_HALF_EVEN` (round-to-even) to validate cost values. For example, $1037.73 USD converts exactly to RM 4,669.78 MYR based on exact `Decimal` banker's rounding behavior, replacing float-based representations to prevent precision loss.
