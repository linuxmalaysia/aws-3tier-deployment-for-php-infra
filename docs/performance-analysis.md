...
| **2,500 VU** | High-Performance Prod | **$1,068.33 USD** | **FAIL (Bottleneck)** | RDS MariaDB Query CPU & PostgreSQL storage write sync | [2,500 VU Sizing Specs](performance-testing.html#-2500-vu--high-performance-production-model-high-concurrency) |
| **5,000 VU** | Heavy Concurrency Prod | **$1,850.00 USD** | **PASS (Optimized)** | Resolving 2,500 VU issues via custom indexing & PIOPS | [5,000 VU Sizing Specs](performance-testing.html#-5000-vu--heavy-concurrency-production-model-optimized-scale-up) |
| **10,000 VU** | Extreme Concurrency Prod | **$3,650.00 USD** | **PASS (Maximum Scale)** | WAF rules overhead, PHP-FPM / Nginx worker limits | [10,000 VU Sizing Specs](performance-testing.html#-10000-vu--extreme-concurrency-production-model-maximum-scale) |
...
* **Date & End Time:** October 6, 2025, 10:30 PM
...
* Maintain the standard cost-saving `t4g.micro` and `db.t4g.micro` setup for development staging as described in the **[100 VU Costing Guide](performance-testing.html#100-vu--baseline-staging-and-development-model)**.
...
#### D. Recommendations & Sizing Mapping
* To support workloads up to 500 VU reliably in staging and QA environments, adopt the specifications in the **[500 VU Sizing Specs](performance-testing.html#-500-vu--cost-optimized-staged-model)**.
...
* **Database Layer (RDS MariaDB):** **FAIL (Bottleneck)**. Database load surged past the system capacity limit, locking the vCPU pool at **8.0 AAS** (exhausting vCPU capabilities).
...
* **Problem:** Average Active Sessions (AAS) spiked to the 8.0 vCPU limit on the `db.m7g.xlarge` instance class, causing extensive query timeouts.
...
#### D. Recommendations & Sizing Mapping
* Immediately execute database tuning and storage upgrades. It is critical to scale the database specs to `db.m7g.2xlarge` with Provisioned IOPS as detailed in the **[2,500 VU Sizing Specs](performance-testing.html#-2500-vu--high-performance-production-model-high-concurrency)**.
* **Composite Index Remediation:**
  ...
  - Create a composite index on the PostgreSQL `parking` table: `idx_parking_lookup (user_id, start_ts DESC, end_ts)`.
...
* **Ramp-up Duration:** 25 minutes
* **Steady-State Duration:** 35 minutes
...
#### D. Recommendations & Sizing Mapping
* Deploy the optimized architecture outlined in the **[5,000 VU Sizing Specs](performance-testing.html#-5000-vu--heavy-concurrency-production-model-optimized-scale-up)**.
...
#### D. Recommendations & Sizing Mapping
* Deploy the extreme high-capacity configurations specified in the **[10,000 VU Sizing Specs](performance-testing.html#-10000-vu--extreme-concurrency-production-model-maximum-scale)**.
...
* **Nginx & PHP-FPM Tuning:**
  ...
  - Set the PHP-FPM process manager (`pm.max_children`) to **120+** with dynamic worker thread allocation.
...
4. **Implement VPC S3 Gateway Endpoints:** Configure S3 Gateway Endpoints inside the private subnets to bypass NAT Gateway data processing charges ($0.045/GB) entirely for all database backup transfers.
