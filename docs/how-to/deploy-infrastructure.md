...
First, make sure that `terraform.tfvars` exists in the `terraform/` directory.
...
Run the `./scripts/deploy.sh` script from the root of the repository.
...
3. Automatically formats all OpenTofu configuration files using `tofu fmt -recursive`.
...
**Solution:**
Force-unlock the state file using the lock ID printed in your terminal error log:
