...
<<<<<<< HEAD
Elasticsearch/OpenSearch indexers require increased memory map limits, memory locking, and file descriptor boundaries.
=======
...
<<<<<<< HEAD
Create a systemd service drop-in configuration for `wazuh-indexer.service`:

```ini
# Create override directory for systemd service limits
...
Configure memory locking in `/etc/wazuh-indexer/wazuh-indexer.yml`:

```yaml
bootstrap.memory_lock: true
```

Set JVM heap options in `/etc/wazuh-indexer/jvm.options` (e.g. 4 GiB initial and maximum heap):
...
Reload systemd and restart indexer service:
...
<<<<<<< HEAD
# Add required operational ports (including SSH port 22/tcp) to wazuh-mgmt zone
sudo firewall-cmd --permanent --zone=wazuh-mgmt --add-port=22/tcp
=======
...
<<<<<<< HEAD
For rootless Podman execution in WSL2, update unprivileged host port mappings in `docker-compose.yml` (e.g., mapping `8443:5601` for Dashboard HTTPS and `5514:514/udp` for syslog events):
...
Before starting the containers for the first time, perform password replacement in `docker-compose.yml` (`INDEXER_PASSWORD`, `DASHBOARD_PASSWORD`, and `API_PASSWORD`) or execute a forced recreate after updating environment variables:
...
<<<<<<< HEAD
To complete password rotation on containerized deployments, generate a new bcrypt password hash for `admin` and `kibanaserver`, update `internal_users.yml` and dependent credentials (`wazuh.yml` for `wazuh-wui`), restart affected containers, and execute `securityadmin.sh` inside the indexer container:

```bash
podman exec -it single-node_wazuh.indexer_1 /usr/share/wazuh-indexer/plugins/opensearch-security/tools/securityadmin.sh \
  -cd /usr/share/wazuh-indexer/plugins/opensearch-security/securityconfig/ \
  -icl -nhnv \
  -cacert /usr/share/wazuh-indexer/config/root-ca.pem \
  -cert /usr/share/wazuh-indexer/config/admin.pem \
  -key /usr/share/wazuh-indexer/config/admin-key.pem
```
...
<<<<<<< HEAD
   Rotate Wazuh API credentials with `wazuh-passwords-tool.sh -u wazuh-wui -p <PASSWORD>` or `--api` alongside indexer rotation using administrator credentials; then update dashboard configuration for `wazuh-wui` in `/usr/share/wazuh-dashboard/data/wazuh/config/wazuh.yml` and restart dependent services (`systemctl restart wazuh-dashboard`).

2. **Password Rotation (WSL2 / Podman Containers):**
   Do not deploy published default credentials (`SecretPassword1!`). Replace `INDEXER_PASSWORD`, `DASHBOARD_PASSWORD`, and `API_PASSWORD` in `docker-compose.yml`, generate bcrypt password hashes, update `internal_users.yml` and `wazuh.yml`, restart affected containers, and execute `securityadmin.sh` inside the indexer container to apply changes.
...
4. **Automated AWS Backups:** Target the EBS storage volume size specified by your selected AWS deployment profile (50 GB for Option A / `t4g.medium` or 100 GB for Option B / `t4g.large`). Filter backup targets using resource tags (e.g. `Name=wazuh-standalone-ebs`) to ensure backup policies apply conditionally based on profile sizing.
=======
2. **Password Rotation (WSL2 / Podman Containers):**
