...
- **Deployment:** Hosted as a NodeJS/Express containerized service inside private application subnets or on-premises GPU nodes.
- **Key Capabilities:**
...
- **Key Capabilities:**
  ...
  - Native gRPC and REST APIs optimized for storing and querying vector embeddings (such as those generated by BGE-M3 or Qwen3 Embeddings).
...
class AiProcessingService
{
    ...
    {
        $this->liteLlmBaseUrl = getenv('LITELLM_PROXY_URL') ?: '';
        $this->apiKey         = getenv('LITELLM_API_KEY') ?: '';
        $this->flowiseBaseUrl = getenv('FLOWISE_API_URL') ?: '';
        $this->client         = Services::curlrequest();

        if (empty($this->liteLlmBaseUrl) || empty($this->apiKey) || empty($this->flowiseBaseUrl)) {
            throw new UnexpectedValueException('Missing required deployment environment configuration for AI Processing Service.');
        }
    }
    ...
    public function generateChatCompletion(string $prompt, string $model = 'bedrock/qwen3-70b', array $systemContext = []): array
    {
        ...
        try {
            ...
            if ($statusCode !== 200) {
                throw new Exception("LiteLLM Gateway returned HTTP status {$statusCode}");
            }

            $decoded = json_decode($response->getBody(), true, 512, JSON_THROW_ON_ERROR);
            if (!is_array($decoded)) {
                throw new UnexpectedValueException('LiteLLM Gateway response payload is not a valid JSON array.');
            }

            return $decoded;
        } catch (Exception $e) {
            log_message('error', "[AiProcessingService][TraceID: {$correlationId}] Chat completion failed: " . $e->getMessage());
            return [
                'error'          => true,
    ...
    public function executeFlowiseWorkflow(string $chatflowId, string $question, array $overrideConfig = []): array
    {
        ...
        try {
            ...
            $statusCode = $response->getStatusCode();
            if ($statusCode !== 200) {
                throw new Exception("Flowise service returned HTTP status {$statusCode}");
            }

            $decoded = json_decode($response->getBody(), true, 512, JSON_THROW_ON_ERROR);
            if (!is_array($decoded)) {
                throw new UnexpectedValueException('Flowise workflow response payload is not a valid JSON array.');
            }

            return $decoded;
        } catch (Exception $e) {
            log_message('error', "[AiProcessingService][TraceID: {$correlationId}] Flowise workflow execution failed: " . $e->getMessage());
            return [
                'error'          => true,
...
| :--- | :--- | :--- | :--- | :--- |
| **TS-07** | Flowise Visual AI | Apache License 2.0 (Community Code) / Commercially Licensed (Enterprise Components) | Low | Permissive open-source community core with enterprise add-ons isolated. Containerized deployment in private subnets with strict authentication. |
...
The addition of **Flowise**, **Qdrant**, and **LiteLLM** complements our existing **RAGFlow** (DeepDoc visual parsing) and **Langfuse** (tracing) infrastructure. By placing LiteLLM at the edge of our AI processing tier, PHP CodeIgniter applications access a resilient, zero-trust AI engine designed to support **Malaysian Personal Data Protection Act (PDPA) 2010 (Act 709)** requirements (for detailed controls and regulatory boundary proofs, refer to our [AI Agent Data Flow & Zero-Trust Handshake Guide](ragflow-langfuse.html) and [Disaster Recovery & Sovereignty Guide](../executive/dr-options.html)).
