...
For a representative 15-node production cluster, the fully consolidated steady-state CloudWatch observability stack (RUM, Application Signals APM, host metrics with 4 to 12 custom metrics/node, native service metrics, alarms/dashboards) is estimated at **$71.00 – $335.00 USD per month (~RM 319.50 – RM 1,507.50 MYR)** post-trial, based on a reproducible workload model (250k–1M web sessions @ 10–20 RUM events/session = 2.5M–20M events = $25–$200; 5M–20M Application Signals Golden Metrics = $7.50–$30; 10–40 GB optional transaction traces = $3.50–$14; 15 EC2 instances @ 4–12 custom metrics/node across 1–2 network interfaces = 60–180 series = $15–$51; composite alarms = $5.00). Compared to legacy Dynatrace Full-Stack Monitoring estimated at **$870.00 – $1,110.00+ USD per month (~RM 3,915.00 – RM 4,995.00+ MYR)** for 15 host units ($58–$74/host unit/month), this migration yields potential recurring operational savings of **~$535.00 – $1,039.00 USD per month (~RM 2,407.50 – RM 4,675.50 MYR per month)** while eliminating third-party SaaS egress and reclaiming host compute capacity.
...
3. **Compute Agent Overhead:** Third-party agents typically carry materially higher RAM (~200–400 MB) and CPU overhead (~2–5%) than lightweight OpenTelemetry user-space daemons (~15–30 MB RAM, <0.2% CPU in standard user-space daemon baselines on 64-bit Linux EC2 Graviton instances), creating a continuous resource tax across every monitored EC2 instance.
4. **Data Sovereignty & Egress:** Exporting telemetry to third-party SaaS platforms creates unnecessary egress charges and external data dependencies. Native CloudWatch tools retain explicitly configured metrics, traces, and logs inside **AWS Malaysia (`ap-southeast-5`)** (note: browser-to-RUM telemetry client requests travel over the public internet to AWS RUM endpoints, whereas backend telemetry transport remains entirely within the regional AWS network).
...
│  Native CloudWatch Metrics -> Request Counts, HTTP Status Codes, Target Latency  │
└────────────────────────────────────────┬─────────────────────────────────────────┘
                                         │ Distributed HTTP Trace Context
                                         ▼
┌──────────────────────────────────────────────────────────────────────────────────┐
│                   COMPUTE LAYER (EC2 / Graviton / Container)                     │
│  - OTel PHP Zero-Code Instrumentation -> Spans & Traces                          │
│  - CloudWatch Agent / Collector -> Telemetry Export (Metrics, Spans, Logs, OS)   │
└────────────────────────────────────────┬─────────────────────────────────────────┘
                                         │ Regional AWS Telemetry Pipeline
                                         ▼
┌──────────────────────────────────────────────────────────────────────────────────┐
│             CLOUDWATCH APPLICATION SIGNALS & DATA TIER (RDS & Valkey)            │
│  - Application Signals Engine -> SLO Evaluation, Golden Metrics, Service Map    │
│  - Native Service Metrics -> Storage, Read/Write IOPS, Engine CPU, Memory        │
└──────────────────────────────────────────────────────────────────────────────────┘
```
...
* **Core Capabilities:**
  ...
  * **Service Level Objectives (SLOs):** Allows teams to define latency and availability SLOs with automated error-budget burn-rate monitoring and alerting managed natively within Application Signals.
...
* **Core Capabilities:**
  ...
  * **Client Error Tracking:** Captures unhandled JavaScript exceptions and HTTP status codes (HTTP 4xx client errors and HTTP 5xx server errors) across web sessions.
...
| **Host Memory Overhead** | ~200 MB – 400 MB RAM per instance | ~15 MB – 30 MB RAM (OTel user-space agent / CWAgent daemon on 64-bit Linux) | Reclaims host RAM for application compute capacity |
...
| **Client Exception Tracking** | Dynatrace JavaScript Error Analysis | CloudWatch RUM Error Stack Analytics | Grouped JS errors, HTTP 4xx/5xx status codes, and client OS breakdown |
| **Data Boundary & Sovereignty** | Data exported to Dynatrace SaaS / External POPS | Explicitly configured regional endpoints remain in **AWS Malaysia (`ap-southeast-5`)** | Supports regional data residency for AWS-hosted telemetry; browser-to-RUM travels over HTTPS |
| **Network Egress Cost** | Egress charges for telemetry sent outside AWS | Eliminates third-party SaaS egress; regional AWS service endpoints route within AWS | Avoids third-party telemetry export fees for backend compute nodes |
...
| **15-Host Cluster Monthly Cost** | **$870.00 – $1,110.00+ USD** (~RM 3,915 – RM 4,995 MYR) | **$71.00 – $335.00 USD** (~RM 319.50 – RM 1,507.50 MYR) | **60% to 90% direct monthly cost reduction** based on reproducible workload model |
| **Evaluation Trial Offer** | 15-day free trial | **3 Months or until 100 GB trace data ingested / 1M spans indexed** | Extended zero-cost validation window for production workloads |
...
* **Report Contents:**
  * Detailed execution timeline (waterfall chart) showing segment duration across CodeIgniter application execution, database SQL execution, and external API calls (note: ALB request counts and target response times correlate separately via CloudWatch metrics and access logs, as X-Ray trace waterfalls do not include an ALB trace segment).
...
* **Report Contents:**
  * Grouped stack traces for unhandled JavaScript exceptions, HTTP 4xx client status codes, and 5xx server error responses.
...
3. **CloudWatch RUM Web Client (`aws-rum-web`):**
   ...
   * **Function:** Provision the AppMonitor with `EnableXRay: true`, then configure the client snippet with `enableXRay: true`, `enableW3CTraceId: true`, and `addXRayTraceIdHeader: true`. Restrict `urlsToInclude` strictly to required first-party API endpoints, ensuring cross-origin requests allow the `traceparent` or `X-Amzn-Trace-Id` headers in CORS configuration.
...
* **Evaluation Trial Window:** The Application Signals and Transaction Search trial lasts three months per account or ends when either 100 GB of Application Signals data is ingested or 1 million spans are indexed as X-Ray Trace Summaries, whichever occurs first.
...
* **JavaScript & HTTP Error Tracking:** Automatically aggregates unhandled client JavaScript exceptions and HTTP status codes (4xx/5xx).
...
| Monthly Sessions | Est. Events (@10–20/session) | Steady-State Monthly Cost (USD) | Steady-State Monthly Cost (MYR @ 4.50) |
| :---: | :---: | :---: | :---: |
| **250,000 sessions** | 2.5M–5M events | **$25.00 – $50.00** ($15.00 – $40.00 in 1st trial mo) | **RM 112.50 – RM 225.00** |
| **1,000,000 sessions** | 10M–20M events | **$100.00 – $200.00** ($90.00 – $190.00 in 1st trial mo) | **RM 450.00 – RM 900.00** |
...
* **Metric Series Calculation Formula:** Custom network series scale as $4 + 4n$ series per host, where $n$ represents the number of monitored network interfaces.
* **Single Network Interface Baseline (4 OS + 4 Net = 8 custom metrics/node):** For 15 hosts = 120 series (110 billed after 10 free) = **$33.00 USD / month (~RM 148.50 MYR)**.
* **Dual Network Interface Baseline (4 OS + 8 Net = 12 custom metrics/node):** For 15 hosts = 180 series (170 billed after 10 free) = **$51.00 USD / month (~RM 229.50 MYR)**.
...
* **Workload Drivers:** 250k–1M web sessions (10–20 RUM events/session = 2.5M–20M events = $25–$200); 5M–20M Application Signals Golden Metrics = $7.50–$30 + optional 10–40 GB traces ($3.50–$14) = $11–$44; 15 EC2 hosts @ 4–12 custom metrics/node across 1–2 interfaces = 60–180 series (50–170 billed @ $0.30) = $15–$51; 10 operational alarms = $5.00.

| Observability Layer | Workload Driver & Rate Inputs | Monthly Cost (USD) | Monthly Cost (MYR @ 4.50) |
| :--- | :--- | :---: | :---: |
| **CloudWatch RUM** | 250k–1M sessions (2.5M–20M events @ $1.00/100k) | $25.00 – $200.00 | RM 112.50 – RM 900.00 |
| **Application Signals (APM)** | 5M–20M signals ($1.50/1M) + optional 10–40 GB traces ($0.35/GB) | $11.00 – $44.00 | RM 49.50 – RM 198.00 |
| **Host Metrics (Agent)** | 15 EC2 hosts @ 4–12 metrics = 60–180 series (50–170 billed @ $0.30) | $15.00 – $51.00 | RM 67.50 – RM 229.50 |
...
| **Total CloudWatch Suite** | **Full-Stack AWS-Native Observability Envelope** | **~$71.00 – $335.00** | **~RM 319.50 – RM 1,507.50** |
| **Legacy Dynatrace OneAgent** | **15 Host Units @ $58–$74/HU/month** | **~$870.00 – $1,110.00+** | **~RM 3,915.00 – RM 4,995.00+** |

**Net Strategic Impact:** Transitioning to native CloudWatch observability delivers full APM and end-user visibility inside **AWS Malaysia (`ap-southeast-5`)** with an immediate recurring operational saving of **~$535.00 to $1,039.00 USD per month (~RM 2,407.50 to RM 4,675.50 MYR per month)**.
