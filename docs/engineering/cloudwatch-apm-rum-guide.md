...
For a representative 15-node production cluster, the fully consolidated CloudWatch observability stack (RUM, APM, host metrics, native service metrics, alarms/dashboards) is estimated at **$76.00 – $269.00 USD per month (~RM 340.00 – RM 1,210.00 MYR)**, against a current Dynatrace Full-Stack Monitoring spend estimated at **$870.00 – $1,110.00+ USD per month (~RM 3,915.00 – RM 5,000.00+ MYR)** based on 2026 published list pricing for 15 host units. This migration yields a potential recurring operational savings of **~RM 2,700.00 – RM 3,800.00 MYR per month** while keeping all telemetry local and reclaiming host compute capacity.
...
| **Host Telemetry (RAM/Disk)** | Host Units (RAM-based) | Unified CloudWatch Agent (`amazon-cloudwatch-agent`) | Decoupled utility metering ($0.30/custom metric) |
...
CloudWatch Application Signals measures application health through two metered, tiered billing dimensions:

* **Golden Metrics (Signals):** **$1.50 USD per 1 million signals** for the first 100 million signals/month; $0.75 per 1M up to 1B; $0.30 per 1M beyond 1B. (Signals include Request counts, Error counts, Latency data points, and Saturation metrics).
...
* **Evaluation Trial Window:** New accounts receive 3 months free (up to 100 GB trace ingestion or 100 million signals, whichever comes first), providing a zero-cost pilot window.

### Monthly Workload Projections (Application Signals)
...
*(Calculation: $20\text{M signals} \times \frac{\$1.50}{1\text{M}} = \$30.00$; $40\text{ GB} \times \$0.35/\text{GB} = \$14.00$; Total = **$44.00 USD/month**).*
...
### 4.1 RUM Capabilities & Configuration
...
* **Distributed Trace Correlation:** Injects standard W3C / AWS X-Ray trace headers (`X-Amzn-Trace-Id`) into client HTTP calls, linking front-end page loads directly to backend ALB, ECS/EC2, and RDS trace segments.
* **Client Privacy & Anonymization:** Native settings allow IP masking (`anonymizeIP: true`) and PII sanitization before telemetry leaves the client browser.

### 4.2 RUM Cost Model

* **Unit Pricing:** **$1.00 USD per 100,000 data events** ($0.00001 per event).
* **Trial Allocation:** First 1,000,000 events/month free per account trial.
* **Event Volume:** Standard page navigation produces approximately **10 to 20 events** per complete user session (Page Load, Navigation Timing, Web Vitals, Errors).

| Monthly Sessions | Est. Events (@20/session) | Monthly Cost (USD) | Monthly Cost (MYR @ 4.50) |
| :---: | :---: | :---: | :---: |
| **250,000 sessions** | 5,000,000 events | **$50.00** | **RM 225.00** |
| **1,000,000 sessions** | 20,000,000 events | **$200.00** | **RM 900.00** |

---

## 5. Capability & Infrastructure Telemetry Matrix

CloudWatch natively captures **CPU, Memory, Network I/O, and Disk Space** telemetry across all application tiers. Managed database and caching engines emit these streams natively at zero extra cost, whereas EC2 guest instances utilize the open-source **Unified CloudWatch Agent (`amazon-cloudwatch-agent`)** for internal guest OS memory and disk metrics.
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **EC2 Instances** | `CPUUtilization` | `mem_used_percent` | `NetworkIn` / `NetworkOut` | `disk_used_percent` | Unified CloudWatch Agent (RPM/DEB) | Basic metrics **Free**; OS memory/disk are **Custom Metrics** (~$0.30/metric) |
...
* **AWS Pricing:** First 10 custom metrics are **Free Tier**; thereafter, **$0.30 USD per metric/month**.
* **Standard Baseline (4 custom metrics/node):** `mem_used_percent`, `mem_available`, `disk_used_percent`, `disk_free` = **$1.20 USD / instance / month**.
* **For a 15-node cluster:** 60 custom metrics (50 billed after 10 free) = **$15.00 USD / month (~RM 67.50 MYR)**.
...
Combining client-side RUM, Application Signals APM, host metrics, and operational alarms yields a complete, unified AWS Observability budget:
...
| **Host Metrics (Agent)** | EC2 OS Memory & Disk metrics (15 instances @ 4 metrics) | $10.00 – $20.00 | RM 45.00 – RM 90.00 |
...
| **Total CloudWatch Suite** | **Full-Stack AWS-Native Observability Envelope** | **~$76.00 – $269.00** | **~RM 342.00 – RM 1,210.50** |
| **Legacy Dynatrace OneAgent** | **Proprietary Host Units + DEM Packs (15 hosts)** | **~$870.00 – $1,110.00+** | **~RM 3,915.00 – RM 4,995.00+** |

**Net Strategic Impact:** Transitioning to native CloudWatch observability delivers full APM and end-user visibility inside **AWS Malaysia (`ap-southeast-5`)** with an immediate recurring operational saving of **~RM 2,700.00 to RM 3,800.00 MYR per month**.
...
```text
Phase 1: Agent & RUM Provisioning (Week 1)
  ├── Deploy Unified CloudWatch Agent via Ansible / EC2 User Data across 15 nodes.
  ├── Provision CloudWatch RUM App Monitor in ap-southeast-5 with IP anonymization.
  └── Enable AWS Distro for OpenTelemetry (ADOT) auto-instrumentation on application tiers.
