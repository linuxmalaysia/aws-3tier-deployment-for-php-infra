...
> *@jules-agent Morning Jules! The unit test run failed on `test_markdown_relative_links_integrity`. Looks like the relative link in `docs/engineering/architecture.md` pointing to `../executive/dr-options.md` was mistakenly altered. Can you confirm `docs/executive/dr-options.md` is preserved, update the link path, run `python3 scripts/prepare_docs.py`, and check that tests pass?*

**Jules Autonomous Response & Execution:**
1. Jules inspects `docs/engineering/architecture.md` and confirms link pathing.
2. Jules verifies link integrity, confirming `docs/executive/dr-options.md` is preserved and correcting the path spec in `docs/engineering/architecture.md` to `../executive/dr-options.md`.
...
5. Jules replies:
> *All verified! Confirmed that `docs/executive/dr-options.md` is preserved, updated the relative link in `docs/engineering/architecture.md`, formatted front matter using `prepare_docs.py`, and verified that all unit tests pass cleanly.*
...
#### 1. Obtaining the API Key and Authenticating
Obtain a Jules API key from the developer console and perform CLI authentication using interactive login or key-based CLI login:
```bash
# Option A: Interactive browser-based authentication
jules login

# Option B: Token-based authentication using environment variable
export JULES_API_KEY="YOUR_API_KEY"
jules auth login --api-key "$JULES_API_KEY"
```
...
JULES_SESSIONS_URL = "https://jules.googleapis.com/v1alpha/sessions"
JULES_SOURCES_URL = "https://jules.googleapis.com/v1alpha/sources"
...
# 1. Resolve source name for target repository with pagination
target_owner = "linuxmalaysia"
target_repo = "aws-3tier-deployment-for-php-infra"
source_name = None
...
while True:
    ...
    sources_resp = requests.get(JULES_SOURCES_URL, headers=headers, params=params, timeout=30)
    sources_resp.raise_for_status()
    sources_data = sources_resp.json()

    for src in sources_data.get("sources", []):
        gh_repo = src.get("githubRepo", {})
        if gh_repo.get("owner") == target_owner and gh_repo.get("repo") == target_repo:
...
if not source_name:
    raise RuntimeError(
        f"No connected Jules source found matching repository '{target_owner}/{target_repo}'."
    )
...
To verify this documentation guide, enforce fail-fast shell execution, and assert zero file drift across generated LLM assets:
...
# 4. Assert zero file drift across compiled LLM assets
git diff --exit-code docs/llms-full.txt llms-full.txt docs/llms-context.xml llms-context.xml
